...
cat > src/travel_tracker/pipelines/flights_summary.py <<'PY'
...
cat > src/travel_tracker/flights_report_main.py <<'PY'
from __future__ import annotations
...
python3 -m py_compile src/travel_tracker/pipelines/flights_summary.py
python3 -m py_compile src/travel_tracker/flights_report_main.py
echo "[OK] Installed flights summary report: flights_summary.py + flights_report_main.py"

echo ""
echo "[NEXT] Usage:"
echo "  poetry run python -m travel_tracker.flights_report_main --date 2025-12-15 --provider tripcom --top 20"
